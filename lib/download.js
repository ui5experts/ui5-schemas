...
const sdkBaseURIs = {
  ...
  openui5nightly: 'https://openui5nightly.hana.ondemand.com',
...
module.exports.determineVersion = async function determineVersion(options) {
  if (options.sdk === 'openui5nightly') {
    return '';
  }
  if (!options.version) {
    log.info(`Attempting to download latest version of '${options.sdk}'. Determining latest version...`);
    const versionUrl = `${sdkBaseURIs[options.sdk]}/resources/sap-ui-version.json`;
    const sapUiVersion = JSON.parse(await rp(versionUrl));
    log.info(`Latest version of '${options.sdk}' is '${sapUiVersion.version}'`);
    return sapUiVersion.version;
  }
