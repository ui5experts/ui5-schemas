...
// TODO: Return Promise
// TODO: Fix sub-defaults
const main = function main(opts) {
  const options = util.storeOptions(opts);

  createLogger(options.debug);
  log.info(`Preparing '${options.sdk}' schemas in version '${options.version || 'latest'}'...`);

  if (!options.version) {
    // find out version, save version and create latest symlink
  }

  const baseURL = util.getSDKBaseURI(options.sdk) + options.version ? `/${options.version}/` : '/';
  ...
  const allLibsPromise = download.downloadFile();
